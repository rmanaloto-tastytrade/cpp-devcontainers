...
- vcpkg: `vcpkg.json` dependencies include fmt, ms-gsl, highway, quill, boost-ut, and overlay ports (boost-ext-outcome, qlibs-core, stdx, cib).
...
- **vcpkg**: rely on the existing manifests/overlays; add `boost-ut` as the only test framework. No gtest dependency.
