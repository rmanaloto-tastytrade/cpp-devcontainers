...
#
# Usage:
#   CONFIG_ENV_FILE=config/env/devcontainer.env scripts/verify_devcontainer.sh [--image <tag>] [--ssh-port <port>]
# Defaults:
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      cat <<EOF
Usage: $0 [--image <tag>] [--ssh-port <port>]
...
if "${SSH_CMD[@]}" 'echo "ssh user: $(whoami)"; which clang++-p2996 || true; /opt/gcc-15/bin/gcc-15 --version | head -n1 || true; clang++-p2996 --version | head -n1 || true; ninja --version; cmake --version | head -n1; vcpkg version | head -n1; /opt/mrdocs/bin/mrdocs --version | head -n1' 2>/tmp/verify_ssh_err.log; then
